...
def check_test(results, output_file):
    ...
    with open(output_file, 'r') as fin:
        ...
        if test_type == "reachable_states":
            ...
            # ans has no duplicates, so if len(ans) == len(results) and the
            # set of tuples are the same, then ans and results are the same.
            return len(ans) == len(results) and set(ans) == set(results), ["Results (below) do not match answer.", str(results)]
        elif test_type == "simple_machine":
            ans = int(line(fin)[0])
            if results == False:
